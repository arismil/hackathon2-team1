...
COMMON_ENV=(
  ...
  "AZURE_EMBEDDING_API_KEY=secretref:embed-key"
  "AZURE_EMBEDDING_API_VERSION=${AZURE_EMBEDDING_API_VERSION:-${AZURE_OPENAI_API_VERSION:-2023-05-15}}"
...
if [[ -n "${LANGFUSE_AZURE_HOST:-}" ]]; then
  ...
  SECRETS=("aoai-key=${AZURE_OPENAI_API_KEY}" "embed-key=${AZURE_EMBEDDING_API_KEY:-$AZURE_OPENAI_API_KEY}" "langfuse-sk=${LANGFUSE_SECRET_KEY}")
else
  SECRETS=("aoai-key=${AZURE_OPENAI_API_KEY}" "embed-key=${AZURE_EMBEDDING_API_KEY:-$AZURE_OPENAI_API_KEY}")
fi
